...
| 确定性脚本 runtime SDK | 完成 | `WorkflowScriptRuntimeRegistry`、内置 `dsh.expr@1`、稳定 `sortBy`、确定性 `withIndex`、防覆盖 `joinBy`、语义诊断、操作预算/取消/安全 key 测试 | 后续按真实需求扩充 builtin，不引入通用 eval |
