...
- [`packages/workflow/tool-workflow/src/types.ts`](../ref_project/deepseek-harness/packages/workflow/tool-workflow/src/types.ts) 用少量 Session event 记录 run/member 生命周期；[`packages/client/ui-workflow-run`](../ref_project/deepseek-harness/packages/client/ui-workflow-run/README.md) 从日志重建 UI。新 DAG 可沿用“完整 run store + Session 摘要事件”的模式。
