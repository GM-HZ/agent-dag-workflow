...
2. Call `workflow_nodes_list`. Apply the two-level extension rule: select a returned DSH `tool` and use `dsh.tool@1` for every ordinary external call; select an exact custom `type@version` Node only when the workflow needs lifecycle semantics that a Tool cannot express. Obey all config/input/output schemas. Inspect `scriptRuntimes` before choosing `core.script@1`; never invent a Tool, Node, or language id.
...
6. Call `workflow_validate`. Fix only deterministic structural issues automatically. For unknown Tools/Nodes, ambiguous bindings, changed side effects, approval placement, or retry decisions, ask the user.
