...
Run Store 在单个事务里追加连续事件并提交对应 checkpoint，使用 expected sequence 防止多个 writer 静默覆盖。数据库启用 `trusted_schema=OFF`、`foreign_keys=ON`、`mmap_size=0`、`synchronous=FULL`，文件数据库使用 WAL。打开已有数据库时验证 application id、schema version 和必需表，不会把未知数据库当作空库初始化；自动化 fixture 覆盖 v1 到当前 schema 的逐步迁移与重复打开。
